...
object Parser {
  ...
  private def lex(input: Seq[String]) = new Iterator[Token] {
    ...
    def next(): Token = if (!shortOptions.isEmpty) {
  ...
  def parse(command: Command, args: Seq[String]): Either[ParseException, CommandLine] = try {
    ...
    def line(command: Command): CommandLine = {
      ...
      def option(): (String, String) = {
        ...
        val name = parts(0)
        val embedded: Maybe[String] = if (parts.size > 1) Some(parts(1)) else None
        ...
        if (opt.argumentRequired) {
          embedded match {
            ...
            case None =>
              fatal(s"option ${opt} requires an argument but ${token.value} found")
          }
          ...
          embedded match {
            case Some(value) => fatal(s"no argument allowed for option $opt (it is set to $value)")
            case None => opt.long -> ""
          }
        }
      }
      ...
      def check(subline: Maybe[CommandLine]): CommandLine = {
        if (!remainingParameters.forall(_.required == false)) {
          ...
          fatal(s"subcommand not specified (must be either one of ${missing.mkString(", ")})")
        } else {
          CommandLine(command.name, parsedOptions.toMap ++ parsedParameters.toMap, subline)
        }
      }

      @annotation.tailrec def innerLine(): CommandLine = {
        if (escaping) {
          ...
          if (subcommands.isEmpty) {
            ...
            subcommands.get(token.value) match {
              case None =>
                ...
                fatal(s"subcommand '${token.value}' not found (must be one of ${cmds.mkString(", ")})")
              case Some(_) =>
