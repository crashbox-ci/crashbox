...
case class Option(
                   ...
                   parameter: Maybe[Parameter] = None) extends Definition {
...
case class Command(
  ...
  def subusage(level: Int): String = {
    ...
    val headline = (Seq(name) ++ optionStrings ++ parameterStrings ++ commandStrings).mkString(" ")
    val sublines = commands.map(_.subusage(level+1)).map(line => "    " * (level+1) + line)
